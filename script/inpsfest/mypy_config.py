...
#
# To update, run python3 -m script.inpsfest -p mypy_config
...
def validate(integrations: dict[str, Integration], config: Config) -> None:
    ...
    if _get_strict_typing_path(config).read_text() != strict_typing_content:
        config.add_error(
            "mypy_config",
            "File .strict_typing is not up to date. Run python3 -m script.inpsfest",
            fixable=True,
        )
    ...
    if _get_mypy_ini_path(config).read_text() != mypy_content:
        config.add_error(
            "mypy_config",
            "File mypy.ini is not up to date. Run python3 -m script.inpsfest",
            fixable=True,
        )
