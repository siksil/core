...
async def test_get_system_info_supervisor_not_available(
    ...
    with (
        ...
        patch("inpui.helpers.system_info.is_docker_env", return_value=True),
        patch("inpui.helpers.system_info.is_official_image", return_value=True),
        patch("inpui.helpers.hassio.is_hassio", return_value=True),
        patch.object(hassio, "get_info", return_value=None),
        patch("inpui.helpers.system_info.cached_get_user", return_value="root"),
    ):
        info = await async_get_system_info(hass)
        assert isinstance(info, dict)
        assert info["version"] == current_version
...
async def test_container_installationtype(hass: HomeAssistant) -> None:
    ...
    with (
        ...
        patch("inpui.helpers.system_info.is_docker_env", return_value=True),
        patch(
            "inpui.helpers.system_info.is_official_image", return_value=False
        ),
        patch("inpui.helpers.system_info.cached_get_user", return_value="user"),
    ):
        info = await async_get_system_info(hass)
        assert info["installation_type"] == "Unsupported Third Party Container"
...
async def test_getuser_oserror(hass: HomeAssistant, error: Exception) -> None:
    ...
    with patch("inpui.helpers.system_info.cached_get_user", side_effect=error):
        info = await async_get_system_info(hass)
