...
from inpui.components.smarty.const import DOMAIN
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "inpui.components.smarty.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_smarty() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "inpui.components.smarty.coordinator.Smarty",
            autospec=True,
        ) as mock_client,
        patch(
            "inpui.components.smarty.config_flow.Smarty",
            new=mock_client,
        ),
    ):
