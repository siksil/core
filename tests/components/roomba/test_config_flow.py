...
from inpui.components.roomba import config_flow
from inpui.components.roomba.const import (
    CONF_BLID,
    CONF_CONTINUOUS,
    DEFAULT_DELAY,
    DOMAIN,
...
async def test_form_user_discovery_and_password_fetch(hass: HomeAssistant) -> None:
    ...
    with patch(
        "inpui.components.roomba.config_flow.RoombaDiscovery", _mocked_discovery
    ):
    ...
    with (
        patch(
            "inpui.components.roomba.config_flow.RoombaFactory.create_roomba",
            return_value=mocked_roomba,
        ),
        patch(
            "inpui.components.roomba.config_flow.RoombaPassword",
            _mocked_getpassword,
        ),
        patch(
            "inpui.components.roomba.async_setup_entry",
            return_value=True,
        ) as mock_setup_entry,
    ):
...
async def test_form_user_no_devices_found_discovery_aborts_already_configured(
    ...
    with patch(
        "inpui.components.roomba.config_flow.RoombaDiscovery",
