...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "inpui.components.solaredge.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_solaredge_api_fixture() -> Generator[Mock]:
    ...
    with (
        patch(
            "inpui.components.solaredge.config_flow.aiosolaredge.SolarEdge",
            return_value=api,
        ),
        patch(
            "inpui.components.solaredge.SolarEdge",
            return_value=api,
        ),
    ):
        yield api
...
def mock_solaredge_web_api_fixture() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "inpui.components.solaredge.config_flow.SolarEdgeWeb", autospec=True
