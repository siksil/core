...
SETUP_ENTRY_PATCHER = patch(
    "inpui.components.starlink.async_setup_entry", return_value=True
)

LOCATION_DATA_SUCCESS_PATCHER = patch(
    "inpui.components.starlink.coordinator.location_data",
    return_value=json.loads(load_fixture("location_data_success.json", "starlink")),
)

SLEEP_DATA_SUCCESS_PATCHER = patch(
    "inpui.components.starlink.coordinator.get_sleep_config",
    return_value=json.loads(load_fixture("sleep_data_success.json", "starlink")),
)

STATUS_DATA_TARGET = "inpui.components.starlink.coordinator.status_data"
...
HISTORY_STATS_SUCCESS_PATCHER = patch(
    "inpui.components.starlink.coordinator.history_stats",
    return_value=json.loads(load_fixture("history_stats_success.json", "starlink")),
)

DEVICE_FOUND_PATCHER = patch(
    "inpui.components.starlink.config_flow.get_id", return_value="some-valid-id"
)

NO_DEVICE_PATCHER = patch(
    "inpui.components.starlink.config_flow.get_id", return_value=None
)
