...
from inpui.components.humidifier import DOMAIN, const, device_condition
from inpui.const import ATTR_MODE, STATE_OFF, STATE_ON, EntityCategory
...
async def test_if_state_legacy(
    ...
    assert len(service_calls) == 1
    assert service_calls[0].data["some"] == "is_mode - event - test_event1"
